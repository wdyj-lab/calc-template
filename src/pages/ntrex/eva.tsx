...
const EvaPage = () => {
  ...
  const formResolver = yup.object().shape({
    ...
    adhesionType: yup
      .mixed()
      .oneOf(Object.keys(AdhesionType))
      .required("접착 여부를 선택해주세요."),
  ...
  const priceFindKey = `${watch("foundationType")}-${watch("adhesionType")}: ${watch("width")}x${watch(
    "height"
  )}x${watch("thickness")}`;
  ...
  const foundationAdhesionOptions = () => {
    switch (watch("foundationType")) {
      case "EVA25":
        return adhesionOptions;
      case "EVA50":
        return adhesionOptions;
      default:
        return [];
        ...
        <FoundationTypeArea>
          <FoundationTypeContainer>
            <FoundationTypeImageBox
              isActive={watch("foundationType") === "EVA25"}
              onClick={() => handleFoundationTypeClick("EVA25")}
            >
              <Image
                src={"/assets/ntrex/EVA 25.jpg"}
                alt={"EVA 25"}
                layout={"fill"}
                objectFit={"contain"}
              />
            </FoundationTypeImageBox>
            <FoundationTypeName isActive={watch("foundationType") === "EVA25"}>
              {FoundationType.EVA25}
            </FoundationTypeName>
          </FoundationTypeContainer>
          <FoundationTypeContainer>
            <FoundationTypeImageBox
              isActive={watch("foundationType") === "EVA50"}
              onClick={() => handleFoundationTypeClick("EVA50")}
            >
              <Image
                src={"/assets/ntrex/EVA 50.jpg"}
                alt={"EVA 50"}
                layout={"fill"}
                objectFit={"contain"}
              />
            ...
            <FoundationTypeName
              isActive={watch("foundationType") === "EVA50"}
            >
              {FoundationType.EVA50}
            </FoundationTypeName>
          </FoundationTypeContainer>
        </FoundationTypeArea>
        <OptionArea>
          <div>
            <FormLabel>{optionTitleObj(currentType)?.adhesionTitle}</FormLabel>
            <OptionRow>
              <SizeWrapper>
                <DropdownV2
                  width="100%"
                  ...
                  options={foundationAdhesionOptions()}
                  placeholder={`${optionTitleObj(currentType).adhesion} 선택`}
                  {...getSimpleProps({
                    key: "adhesionType",
                    setValue,
                    watch,
                  ...
                  hintText={`${optionTitleObj(currentType).adhesion}를 선택해주세요`}
                />
              </SizeWrapper>
