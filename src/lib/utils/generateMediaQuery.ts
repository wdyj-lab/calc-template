import breakpoints, { Breakpoints } from 'lib/types/breakpoints';

type CompareOperator = '>' | '>=' | '<=' | '<';

const foundations = { breakpoints } as { breakpoints: Breakpoints };
...
const generateMediaQuery = (
  ...
  [mediaType, generateMediaQueryForSize(compareOperator, viewport)]
    .filter((x) => x)
    .join(' and ');
